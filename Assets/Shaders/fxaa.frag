...
void main()
{
  vec3 colorCenter = texture(texture0, frag_uv).rgb;

  // Luma at the current fragment
  float lumaCenter = rgb2luma(colorCenter);

  // Luma at the four direct neighbours of the current fragment.
  float lumaDown = rgb2luma(textureOffset(texture0, frag_uv, ivec2(0, -1)).rgb);
  float lumaUp = rgb2luma(textureOffset(texture0, frag_uv, ivec2(0, 1)).rgb);
  float lumaLeft = rgb2luma(textureOffset(texture0, frag_uv, ivec2(-1, 0)).rgb);
  ...
  float lumaEnd1 = rgb2luma(texture(texture0, uv1).rgb);
  float lumaEnd2 = rgb2luma(texture(texture0, uv2).rgb);
  ...
  if (!reachedBoth)
  {
    ...
    for (int i = 2; i < ITERATIONS; i++)
    {
      ...
      if (!reached1)
      {
        lumaEnd1 = rgb2luma(texture(texture0, uv1).rgb);
        lumaEnd1 = lumaEnd1 - lumaLocalAverage;
      }
      // If needed, read luma in opposite direction, compute delta.
      if (!reached2)
      {
        lumaEnd2 = rgb2luma(texture(texture0, uv2).rgb);
