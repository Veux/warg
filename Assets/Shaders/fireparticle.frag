...
void main()
{
  ...
  float costime = 0.5*(1.+cos(speed*time));
   ...
   //variance = (0.2+sintime)*sin((speed*time)+frag_world_position.x*4.) + (0.2+costime)*cos((speed*time)+frag_world_position.y*4.);

  //variance = 1.f-variance;
  //variance = .325f*variance;
  //vec2 randv = vec2(random(frag_world_position.x+frag_world_position.z),random(frag_world_position.y+frag_world_position.z));
