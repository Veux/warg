...
/*
 ...
 NavFlags_EnableKeyboard), the io.WantCaptureKeyboard flag will be set. For more
 ...
 etc. functions. Please reach out if you think the game vs navigation input
 ...
 io.NavFlags. Enabling ImGuiNavFlags_MoveMouse instructs dear imgui to move your
 mouse cursor along with navigation movements. When enabled, the NewFrame()
 function may alter 'io.MousePos' and set 'io.WantMoveMouse' to notify you that
 it wants the mouse cursor to be moved. When that happens your back-end NEEDS to
 move the OS or underlying mouse cursor on the next frame. Some of the binding
 in examples/ do that. (If you set the ImGuiNavFlags_MoveMouse flag but don't
 honor 'io.WantMoveMouse' properly, imgui will misbehave as it will see your
 mouse as moving back and forth.) (In a setup when you may not have easy control
 over the mouse cursor, e.g. uSynergy.c doesn't expose moving remote mouse
 cursor, you may want to set a boolean to ignore your other external mouse
 ...
 which your renderer can use. You may call ImGui::ShowMetricsWindow() to explore
 active draw lists and visualize/understand how the draw data is generated. It
 ...
 label? A: Yes. A primer on labels and the ID stack...
 ...
 within string itself. Use "##" to pass a complement to the ID that won't be
 visible to the end-user. This helps solving the simple collision cases when you
 know which items are going to be created.
 ...
 integer value. Remember that ID are formed from the concatenation of
 _everything_ in the ID stack!
 ...
 ImDrawList API) A: - You can create a dummy window. Call Begin() with
 NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flag, push a
 ImGuiCol_WindowBg with zero alpha, then retrieve the ImDrawList* via
...
static void SetWindowCollapsed(
...
static void AddWindowToDrawData(
    ImVector<ImDrawList *> *out_list, ImGuiWindow *window);
...
static void DataTypeApplyOp(
    ImGuiDataType data_type, int op, void *value1, const void *value2);
static bool DataTypeApplyOpFromText(const char *buf,
    const char *initial_value_buf, ImGuiDataType data_type, void *data_ptr,
...
static void SetNavIDAndMoveMouse(
...
static bool NavScoreItem(ImGuiNavMoveResult *result, ImRect cand)
{
  ...
  if (ImGui::IsMouseHoveringRect(cand.Min, cand.Max))
  {
    ...
    g.OverlayDrawList.AddRectFilled(cand.Max - ImVec2(4, 4),
  ...
  // however lead to disconnected graphs when nodes are very spaced out on
...
bool ImGui::FocusableItemRegister(
    ImGuiWindow *window, ImGuiID id, bool tab_stop)
{
  ImGuiContext &g = *GImGui;
...
ImGuiIO &ImGui::GetIO()
{
  IM_ASSERT(GImGui != NULL && "No current context. Did you call "
                              "ImGui::CreateContext() or "
...
static ImGuiWindow *FindWindowNavigable(
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode)
{
  ...
  if (mode == ImGuiInputReadMode_Repeat)
    return (float)CalcTypematicPressedRepeatAmount(t, t - g.IO.DeltaTime,
        g.IO.KeyRepeatDelay * 0.80f, g.IO.KeyRepeatRate * 0.80f);
  if (mode == ImGuiInputReadMode_RepeatSlow)
    return (float)CalcTypematicPressedRepeatAmount(t, t - g.IO.DeltaTime,
        g.IO.KeyRepeatDelay * 1.00f, g.IO.KeyRepeatRate * 2.00f);
  if (mode == ImGuiInputReadMode_RepeatFast)
    return (float)CalcTypematicPressedRepeatAmount(t, t - g.IO.DeltaTime,
        g.IO.KeyRepeatDelay * 0.80f, g.IO.KeyRepeatRate * 0.30f);
  return 0.0f;
}
...
static bool IsNavInputPressedAnyOfTwo(
  ...
  if (start_windowing_with_gamepad || start_windowing_with_keyboard)
    if (ImGuiWindow *window =
            g.NavWindow ? g.NavWindow
                        : FindWindowNavigable(g.Windows.Size - 1, -INT_MAX, -1))
    {
...
// updated.
static void NavScrollToBringItemIntoView(
    ImGuiWindow *window, ImRect &item_rect_rel)
{
      ...
      (g.NavMoveResultLocal.ID != 0 || g.NavMoveResultOther.ID != 0))
  {
    ...
    if (g.NavMoveResultOther.ID != 0 &&
        g.NavMoveResultOther.Window->ParentWindow ==
            g.NavWindow) // Maybe entering a flattened child? In this case solve
        ...
        !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs))
    {
      ...
      if ((allowed_dir_flags & (1 << ImGuiDir_Down)) &&
          IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadDown,
              ImGuiNavInput_KeyDown_, ImGuiInputReadMode_Repeat))
...
static void ImGui::UpdateMovingWindow()
{
      ...
      g.ActiveIdSource == ImGuiInputSource_Mouse)
  {
    ...
    // so that ActiveIdWindow == MovingWindow and ActiveId ==
...
void ImGui::NewFrame()
{
  ...
  IM_ASSERT(g.IO.Fonts->Fonts.Size > 0 && "Font Atlas not built. Did you call "
      ...
      (g.IO.MouseWheel != 0.0f || g.IO.MouseWheelH != 0.0f))
  {
    ...
    if (g.IO.MouseWheelH != 0.0f && scroll_allowed)
    {
      ...
      if (!g.IO.KeyCtrl &&
        ...
        SetWindowScrollX(
            window, window->Scroll.x - g.IO.MouseWheelH * scroll_amount);
    }
  }
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext *imgui_ctx,
    ImGuiSettingsHandler *handler, ImGuiTextBuffer *buf)
{
...
static void AddDrawListToDrawData(
    ImVector<ImDrawList *> *out_render_list, ImDrawList *draw_list)
{
...
static void SetupDrawData(
    ImVector<ImDrawList *> *draw_lists, ImDrawData *out_draw_data)
{
...
void ImGui::EndFrame()
{
      ...
      ImLengthSqr(g.OsImePosRequest - g.OsImePosSet) > 0.0001f)
  {
    g.IO.ImeSetInputScreenPosFn(
        (int)g.OsImePosRequest.x, (int)g.OsImePosRequest.y);
...
void ImGui::Render()
{
  ...
  if (g.Style.Alpha > 0.0f)
  {
    ...
    if (window_to_render_front_most && window_to_render_front_most->Active &&
        window_to_render_front_most->HiddenFrames <= 0) // NavWindowingTarget is
                                                        // always temporarily
                                                        // displayed as the
                                                        // front-most window
    ...
    if (g.IO.MouseDrawCursor && g.IO.Fonts->GetMouseCursorTexData(g.MouseCursor,
    ...
    const ImVec2 &align, const ImRect *clip_rect)
{
  ...
  if (need_clipping)
  {
    ...
    window->DrawList->AddText(g.Font, g.FontSize, pos,
        GetColorU32(ImGuiCol_Text), text, text_display_end, 0.0f,
        &fine_clip_rect);
  }
  else
  {
    window->DrawList->AddText(g.Font, g.FontSize, pos,
        GetColorU32(ImGuiCol_Text), text, text_display_end, 0.0f, NULL);
  }
  if (g.LogEnabled)
    LogRenderedText(&pos, text, text_display_end);
}
...
static ImVec2 FindBestWindowPosForPopup(const ImVec2 &ref_pos,
    const ImVec2 &size, ImGuiDir *last_dir, const ImRect &r_avoid,
    ImGuiPopupPositionPolicy policy = ImGuiPopupPositionPolicy_Default)
{
  ...
  r_outer.Expand(ImVec2((size.x - r_outer.GetWidth() > safe_padding.x * 2)
                            ? -safe_padding.x
                            : 0.0f,
  ...
  if (policy == ImGuiPopupPositionPolicy_ComboBox)
  {
         ...
         n++)
    {
      ...
      if (dir == ImGuiDir_Right)
        pos = ImVec2(
            r_avoid.Min.x, r_avoid.Min.y - size.y); // Above, Toward Right
      if (dir == ImGuiDir_Left)
        pos =
    ...
    const char *name, ImVec2 size, ImGuiWindowFlags flags)
{
  ...
  if (flags & ImGuiWindowFlags_NoBringToFrontOnFocus)
    g.Windows.insert(
        g.Windows.begin(), window); // Quite slow but rare and only once
  else
  ...
  if (first_begin_of_the_frame)
  {
    ...
    if (parent_window && !(flags & ImGuiWindowFlags_Modal) &&
      ...
      window->RootWindowForTitleBarHighlight = window->RootWindowForTabbing =
          parent_window->RootWindowForTitleBarHighlight; // Same value in master
                                                         // branch, will differ
                                                         // for docking
        ...
        rect_to_avoid =
            ImRect(parent_menu->Pos.x + horizontal_overlap, -FLT_MAX,
                parent_menu->Pos.x + parent_menu->Size.x - horizontal_overlap -
                    parent_menu->ScrollbarSizes.x,
                FLT_MAX);
      window->PosFloat = FindBestWindowPosForPopup(window->PosFloat,
          window->Size, &window->AutoPosLastDirection, rect_to_avoid);
    }
      ...
      if (window->AutoPosLastDirection == ImGuiDir_None)
        window->PosFloat =
            ref_pos + ImVec2(2, 2); // If there's not enough room, for tooltip
                                    // we prefer avoiding the cursor at all cost
                                    // even if it means that part of the tooltip
                                    // won't be visible.
    }
    ...
    if ((flags & ImGuiWindowFlags_Modal) != 0 &&
      ...
      window->DrawList->AddRectFilled(viewport_rect.Min, viewport_rect.Max,
          GetColorU32(
              ImGuiCol_ModalWindowDarkening, g.ModalWindowDarkeningRatio));
    ...
    const bool title_bar_is_highlight =
        want_focus ||
        (g.NavWindow && window->RootWindowForTitleBarHighlight ==
    ...
    if (!(flags & ImGuiWindowFlags_NoTitleBar))
    {
      ...
      if (!(flags & ImGuiWindowFlags_NoCollapse))
      {
        ...
        if (ButtonBehavior(bb, id, NULL, NULL))
          window->CollapseToggleWanted = true; // Defer collapsing to next frame
                                               // as we are too far in the
                                               // Begin() function
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
  ...
  if (horizontal)
    ...
    window_rounding_corners =
        ...
        (other_scrollbar ? 0 : ImDrawCornerFlags_BotRight);
  window->DrawList->AddRectFilled(bb.Min, bb.Max,
      GetColorU32(ImGuiCol_ScrollbarBg), window->WindowRounding,
      window_rounding_corners);
  bb.Expand(ImVec2(
...
bool ImGui::ArrowButton(
    ImGuiID id, ImGuiDir dir, ImVec2 padding, ImGuiButtonFlags flags)
{
  ImGuiContext &g = *GImGui;
  ImGuiWindow *window = g.CurrentWindow;
  if (window->SkipItems)
  ...
  const ImRect bb(window->DC.CursorPos,
      window->DC.CursorPos +
          ImVec2(g.FontSize + padding.x * 2.0f, g.FontSize + padding.y * 2.0f));
  ItemSize(bb, style.FramePadding.y);
  if (!ItemAdd(bb, id))
    return false;
    ...
    const char *label, const char *label_end)
{
  ...
  ImRect frame_bb = ImRect(
      window->DC.CursorPos, ImVec2(window->Pos.x + GetContentRegionMax().x,
  ...
  else
  {
    ...
    if (flags & ImGuiTreeNodeFlags_Bullet)
      RenderBullet(frame_bb.Min + ImVec2(text_offset_x * 0.5f,
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect &aabb,
    const char *label, ImGuiDataType data_type, void *data_ptr, ImGuiID id,
    int decimal_precision)
{
  ...
  if (text_value_changed)
    return DataTypeApplyOpFromText(buf,
        GImGui->InputTextState.InitialText.begin(), data_type, data_ptr, NULL);
  return false;
}
...
bool ImGui::SliderBehavior(const ImRect &frame_bb, ImGuiID id, float *v,
    float v_min, float v_max, float power, int decimal_precision,
    ...
    float v_min, float v_max, const char *display_format, float power)
{
  ...
  bool value_changed = SliderBehavior(frame_bb, id, v, v_min, v_max, power,
      decimal_precision, ImGuiSliderFlags_Vertical);
...
bool ImGui::DragBehavior(const ImRect &frame_bb, ImGuiID id, float *v,
    float v_speed, float v_min, float v_max, int decimal_precision, float power)
{
  ImGuiContext &g = *GImGui;
  const ImGuiStyle &style = g.Style;
...
bool ImGui::DragFloatRange2(const char *label, float *v_current_min,
    ...
    const char *display_format, const char *display_format_max, float power)
{
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
    ...
    float scale_max, ImVec2 graph_size)
{
  ...
  if (values_count > 0)
  {
    ...
    ImVec2 tp0 = ImVec2(t0,
        1.0f - ImSaturate((v0 - scale_min) * inv_scale)); // Point in the
                                                          // normalized space of
                                                          // our target
                                                          // rectangle
...
bool ImGui::Checkbox(const char *label, bool *v)
{
  ...
  if (*v)
  {
    ...
    RenderCheckMark(check_bb.Min + ImVec2(pad, pad),
        GetColorU32(ImGuiCol_CheckMark), check_bb.GetWidth() - pad * 2.0f);
  }
    ...
    ImGuiTextEditCallback callback, void *user_data)
{
    ...
    else if (is_cut || is_copy)
    {
      ...
      if (io.SetClipboardTextFn)
      {
        ...
        ImTextStrToUtf8(edit_state.TempTextBuffer.Data,
            edit_state.TempTextBuffer.Size, edit_state.Text.Data + ib,
  ...
  if (g.ActiveId == id)
  {
    ...
    if (apply_edit_back_to_user_buffer)
    {
      ...
      if (is_editable)
      {
        ...
        ImTextStrToUtf8(edit_state.TempTextBuffer.Data,
            edit_state.TempTextBuffer.Size, edit_state.Text.Data, NULL);
      }
        ...
        if (event_flag)
        {
          ...
          if (callback_data.BufDirty)
          {
            ...
            edit_state.CurLenW = ImTextStrFromUtf8(edit_state.Text.Data,
                edit_state.Text.Size, callback_data.Buf, NULL);
            edit_state.CurLenA =
  ...
  if (g.ActiveId == id || is_currently_scrolling)
  {
    ...
    draw_window->DrawList->AddText(g.Font, g.FontSize,
        render_pos - render_scroll, GetColorU32(ImGuiCol_Text), buf_display,
...
bool ImGui::InputIntN(
    const char *label, int *v, int components, ImGuiInputTextFlags extra_flags)
{
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
...
bool ImGui::InputInt2(
    const char *label, int v[2], ImGuiInputTextFlags extra_flags)
{
  return InputIntN(label, v, 2, extra_flags);
}

bool ImGui::InputInt3(
    const char *label, int v[3], ImGuiInputTextFlags extra_flags)
{
  return InputIntN(label, v, 3, extra_flags);
}

bool ImGui::InputInt4(
    const char *label, int v[4], ImGuiInputTextFlags extra_flags)
{
    ...
    const char *label, const char *preview_value, ImGuiComboFlags flags)
{
  ...
  if (ImGuiWindow *popup_window = FindWindowByName(name))
    if (popup_window->WasActive)
    {
      ...
      ImVec2 pos = FindBestWindowPosForPopup(frame_bb.GetBL(), size_expected,
    ...
    int items_count, int popup_max_height_in_items)
{
  ...
  if (popup_max_height_in_items != -1 && !g.NextWindowData.SizeConstraintCond)
  {
    ...
    SetNextWindowSizeConstraints(
        ImVec2(0, 0), ImVec2(FLT_MAX, popup_max_height));
  }
    ...
    ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
  ...
  if (!ItemAdd(
          bb_with_spacing, (flags & ImGuiSelectableFlags_Disabled) ? 0 : id))
  {
  ...
  if (hovered || selected)
  {
    ...
    RenderNavHighlight(bb_with_spacing, id,
    ...
    const char *label, const char *shortcut, bool selected, bool enabled)
{
  ...
  else
  {
    ...
    pressed = Selectable(label, false,
        flags | ImGuiSelectableFlags_DrawFillAvailWidth, ImVec2(w, 0.0f));
...
bool ImGui::BeginMenuBar()
{
  ...
  // of rounding to Max.x to that text in long menus don't tend to display over
...
void ImGui::RenderColorRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max,
    ImU32 col, float grid_step, ImVec2 grid_off, float rounding,
...
static void ColorPickerOptionsPopup(
    ...
    ImGuiColorEditFlags flags, const float *ref_col)
{
  ...
  else if (flags & ImGuiColorEditFlags_PickerHueBar)
  {
    ...
    draw_list->AddRectFilledMultiColor(picker_pos,
        picker_pos + ImVec2(sv_picker_size, sv_picker_size), IM_COL32_WHITE,
  ...
  if (alpha_bar)
  {
    ...
    RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max,
        IM_COL32(0, 0, 0, 0), bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f));
...
static float OffsetNormToPixels(
    const ImGuiColumnsSet *columns, float offset_norm)
{
...
static float GetColumnWidthEx(
    ImGuiColumnsSet *columns, int column_index, bool before_resize = false)
{
  if (column_index < 0)
    column_index = columns->Current;
    ...
    const char *str_id, int columns_count, ImGuiColumnsFlags flags)
{
  ...
  for (int n = 0; n < columns_count + 1; n++)
  {
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow))
      t = ImMin(t, PixelsToOffsetNorm(columns,
...
void ImGui::EndColumns()
{
  ...
  if (!(columns->Flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
  {
    ...
    for (int n = 1; n < columns->Count; n++)
    {
      ...
      const ImRect column_rect(
          ImVec2(x - column_hw, y1), ImVec2(x + column_hw, y2));
  ...
  if (source_drag_active)
  {
    ...
    if (!(flags & ImGuiDragDropFlags_SourceNoPreviewTooltip))
    {
      ...
      PushStyleColor(ImGuiCol_PopupBg,
          GetStyleColorVec4(ImGuiCol_PopupBg) * ImVec4(1.0f, 1.0f, 1.0f, 0.6f));
      BeginTooltip();
    }
...
void ImGui::ShowMetricsWindow(bool *p_open)
{
  if (ImGui::Begin("ImGui Metrics", p_open))
  {
    ...
    struct Funcs
    {
      ...
      {
        bool node_open =
            ImGui::TreeNode(draw_list, "%s: '%s' %d vtx, %d indices, %d cmds",
             ...
             elem_offset += pcmd->ElemCount, pcmd++)
        {
          ...
          ImGuiListClipper clipper(
              pcmd->ElemCount / 3); // Manually coarse clip our print out of
      ...
      static void NodeWindow(ImGuiWindow *window, const char *label)
      {
        ...
        ImGui::BulletText("Scroll: (%.2f/%.2f,%.2f/%.2f)", window->Scroll.x,
            GetScrollMaxX(window), window->Scroll.y, GetScrollMaxY(window));
    ...
    if (ImGui::TreeNode("Internal state"))
    {
      ...
      ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec)", g.HoveredId,
          ...
          g.HoveredIdTimer); // Data is "in-flight" so depending on when the
