...
struct Scene_Graph
{
  ...
  //
  void set_parent(
      std::weak_ptr<Scene_Graph_Node> ptr, std::weak_ptr<Scene_Graph_Node> desired_parent, bool parent_owned = false);
...
private:
  ...
  void visit_nodes(std::weak_ptr<Scene_Graph_Node> node_ptr, const mat4 &M, std::vector<Render_Entity> &accumulator);
